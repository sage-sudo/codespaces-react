...
class YFinanceVendor extends BaseVendorAdapter {
  ...
  const downloadCode = (codeType) => {
    ...
    const filename = `${codeType}_code.js`;
  ...
  const downloadBundle = () => {
    const bundle = Object.entries(codeExamples).map(([key, example]) => {
      const code = customCode[key] || example.code;
      return `// ========== ${example.title} ==========\n${code}\n\n`;
    }).join('');
    
    const filename = `trading_dashboard_bundle.js`;
